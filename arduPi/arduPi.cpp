...
int getBoardRev(){
	...
	if ((cpu_info = fopen("/proc/cpuinfo","r"))==NULL){
		fprintf(stderr,"Unable to open /proc/cpuinfo. Cannot determine board revision.\n");
		exit(1);
	}
	
	while (fgets (line,120,cpu_info) != EOF){
		sscanf("Revision: %s", c);
		if(c != NULL) break;
	}
	...
	if ( strcmp(id), "0002" == 0 || strcmp(id), "0003" ) {
		bsc0 = bsc_rev1;
		return 1;
	}
...
inline int raspberryPinNumber(int arduinoPin){
	const int piPin[]={0,14,18,23,24,25,4,17,27,22,8,10,9,11};
	
	if (arduinoPin == 8  && REV == 1)
		return 21;
	return piPin[arduinoPin];
}
...
inline pthread_t *getThreadIdFromPin(int pin){
	return idThread[pin];
}
